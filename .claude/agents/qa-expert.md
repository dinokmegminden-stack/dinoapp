...
- **Unlock codes are one-time-use at the DB level.** `unlockCodesService.js`'s guarantee comes from a conditional `UPDATE ... WHERE used_by_player_id IS NULL`, not client logic. A meaningful test here mocks Supabase to return zero rows on the second call and asserts `{ success: false, reason: 'not_found_or_used' }` — not just the happy path.
- **Services never throw.** The established convention (see `creaturesService.js`, `unlockCodesService.js`) is: catch/handle Supabase errors, `console.warn`, return a safe fallback shape. Any new service test should assert the function resolves (never rejects) even when Supabase errors, and returns the documented failure shape.
