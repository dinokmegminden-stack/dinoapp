...
import { getSilhouette } from './components/DinoSilhouettes';
...
function DinoCard({ dino, index, total }) {
  ...
  const svgXml = getSilhouette(dino.nev_tudomanyos, color.accent);
  ...
  return (
    <View style={styles.card}>
      <View style={[styles.cardImageArea, { backgroundColor: color.bg }]}>
        ...
        <SvgXml xml={svgXml} width="85%" height="85%" />
      ...
      <ScrollView style={styles.cardBody} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsGrid}>
          ...
          <StatBox label="Felfedezve" value={`${dino.felfedezés?.ev}`} />
        </View>
        ...
        <Text style={styles.bodyText}>
          {dino.megaltalas_helye?.helyszin} · {dino.megaltalas_helye?.orszag}
        </Text>

        <Text style={styles.sectionLabel}>Felfedező</Text>
        <Text style={styles.bodyText}>{dino.felfedezés?.felfedezo}</Text>

        <Text style={styles.sectionLabel}>Körülmény</Text>
        <Text style={styles.bodyText}>{dino.felfedezés?.korulmeny}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.cardStack}>
        ...
        <Animated.View
          style={[styles.cardWrapper, { transform: [{ translateX: position.x }, { rotate: position.x.interpolate({ inputRange: [-SCREEN_WIDTH, 0, SCREEN_WIDTH], outputRange: ['-6deg', '0deg', '6deg'] }) }] }]}
...
const styles = StyleSheet.create({
  ...
  cardImageArea: { height: 165, alignItems: 'center', justifyContent: 'center', position: 'relative' },
  periodBadge: { position: 'absolute', top: 12, left: 12, backgroundColor: 'rgba(255,255,255,0.88)', borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 0.5 },
  periodBadgeText: { fontSize: 10, fontWeight: '600', letterSpacing: 0.5 },
  cladeBadge: { position: 'absolute', top: 12, right: 12, backgroundColor: 'rgba(255,255,255,0.88)', borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 0.5 },
  cladeBadgeText: { fontSize: 10, fontWeight: '500', color: COLORS.blue },
  counterText: { position: 'absolute', bottom: 10, right: 14, fontSize: 10, color: COLORS.textMuted },
