...
Platform: React Native (Expo), egyszerre fut webes és natív mobil nézetben; webes asztali böngészőben valódi, széles desktop-elrendezést használ (a `Shell` komponens `maxWidth`-je 1100px-ig nyúlik web-asztali nézetben), nincs többé fenntartott hirdetési hely az oldalakon (lásd `Shell` komponens lejjebb).

## 1. Technológiai Stack & Korlátok
- **Keretrendszer:** React Native (Expo, managed workflow).
...
- **Betűtípusok:** a kanonikus család-nevek a `src/constants/theme.js` `FONTS` tokenjében (ami a `src/constants/fonts.js`-t re-exportálja), központilag betöltve az `App.js`-ben (`useAppFonts` → Rokkitt + Inter). Címsorok/anchor: **Rokkitt** (`FONTS.heading` = Rokkitt_700Bold, `FONTS.headingXBold` = Rokkitt_800ExtraBold) — szögletes, kőbe vésett slab serif az őskori hangulathoz. Törzsszöveg és UI: **Inter** (`FONTS.body` = Inter_400Regular a bekezdésekhez/hosszú lexikon-szöveghez, `FONTS.bold`/`FONTS.bodyBold` = Inter_700Bold a gombokhoz, térkép-számokhoz, kvíz-opciókhoz). A dínók **tudományos (latin) neve** továbbra is **kizárólag `Cinzel_700Bold`** (külön töltve a `DailyDinoCard`/`TradingCard`-ban). Vizuális hierarchia opacitással (`TEXT_OPACITY`): fő címsor 100%, leírás 70%, meta/képaláírás 60%. Új komponensnél a `fontFamily`-t explicit a `FONTS` tokenből kell beállítani.
- **Kijelző & Elrendezés:** Az alkalmazás tartalmaz egy `Shell` komponenst a webes nézet támogatásához (asztali böngészőben `maxWidth: 1100px`-es, valódi desktop-szélességű elrendezés, nincs fenntartott hirdetési sáv). Új képernyőket mindig a `Shell` komponensbe kell ágyazni! Fontos: a `Shell`-be ágyazott képernyő saját legkülső (`container`) View-jának explicit `width: '100%'` kell, különben a wide-web módban aktív `alignItems: 'center'` miatt a tartalom a saját content-szélességére zsugorodik ahelyett, hogy kitöltené a rendelkezésre álló helyet (ez a hiba a Dínófutam pályájánál került elő).
